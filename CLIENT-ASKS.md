...
| A-03b | Which mailbox should checklist/chase **drafts** appear in — `visa.lodgement@` (works now) or `project1@` (needs one OAuth)? | 11 Aug | 🟢 **preference, not a blocker.** Building on `visa.lodgement@`; swapping later is a one-field change |
