...
| **A-25** | 🔴 **OFFICE and TEAM for each client — the field that exists in no file you have sent us.** M3 files every client folder by `Office` (BRISBANE) and `Team` (FILIPINO / INDIAN). We checked all four workbooks: neither column exists anywhere. Also: is `LODGEMENT JULY TO PRESENT` all Brisbane? All one team? And **is there an email address list for clients** — we found 3 in 44 rows | 15 Aug | 🔴 **BLOCKS THE IMPORT.** Not ops — data (D-315). Without it the pilot writes an assumed `BRISBANE`/`FILIPINO` into every row and folders land in the wrong team's directory. Without client email, **M4b and M5b cannot be tested on real rows at all** |
