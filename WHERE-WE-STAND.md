...
| 6 | **M9's leftovers** | ⬜ restore trigger `limit` **1 → 10** (dropped for cheap diagnostic runs) · ⬜ add `parseS56Classifications` + `verifyS56Deadlines` to `installDailyTriggers()` · ⬜ **a Make scenario slot** — M3 + M4 take both at go-live and M9 becomes the third, so it cannot run at all (I-21) |
