...
| M9 | Email triage / s56 | ✅ **PROVEN END TO END 19 Aug — real Department email, real row, correct verdict** | Gmail → Claude (`tool_use`) → Sheets row 2 → parser: **1 parsed, 0 UNPARSEABLE, `NOT a Dept request 1`** — a bridging-visa grant correctly NOT forced into the s56 shape. **3 operations per email**, measured. Three silent-failure classes found and fixed getting here: `onerror:Ignore` on the write module turned a total write failure into a green run; `input_schema.type` is a Make form default that is NOT applied at runtime; `toJSON()` does not exist in Make. **58 → 65 tests.** ⬜ Before go-live: delete the test row · restore trigger `limit` 1 → 10 · triggers for `parseS56Classifications`/`verifyS56Deadlines` · **a scenario slot** (I-21, Friday) |
