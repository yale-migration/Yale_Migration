...
| **13** | 🔴 **PROVE THE DAILY TRIGGERS HAVE ACTUALLY BEEN RUNNING** (D-368/D-371). `assignMissingCodes` is confirmed healthy — 4 clean runs after the 23 Aug incident. But `updateFollowUps` 07:00 and `updateEnquiryFollowUps` 08:00 have **never been seen in an Executions list**, and they are the two we describe to the client as "running daily". Apps Script → Executions → filter by function. ⚠️ Executions retains **7 days only** | 0.2 | 🔴 **the claim is unsupported until this is done** |
