...
| M5a | Dormancy detection | ✅ built · 🔴 **TRIGGER STILL NOT INSTALLED — and the guide sent to Rey on 19 Aug claims "every morning" TWICE** | Apps Script, 0 ops · **40/40** (was 36 — 4 lock tests added 19 Aug). 🔴 **`updateFollowUps()` held NO lock** and does read-all → write-back-by-index on MASTER, the same shape as the `removeDemoRows()` bug; M4 writes AE on those rows, so a stale write-back re-stamps CHASE over DRAFTED and M4 redrafts the same email every morning. **Document lock added 19 Aug before any trigger goes in.** ▶ **ACTION: run `installDailyTriggers()` then `verifyDailyTriggers()`** (`scripts/install_daily_triggers.gs`) |
