...
| ~~I-10~~ | 🟢 **ANSWERED 25 Aug — all four `<-- needed` filled, but as FREE TEXT** (`acecqa`, `Bachelor Degree -no skills assessment`, …). ⛔ `Skills Authority` is a locked dropdown that refuses those silently, and the import gate had **never checked that column** — fixed, 10 of 10 covered (D-403). Case-only variants now resolve; sentences stay blank and noted | 4 × 485 checklists | — | 🟢 |
...
| ~~I-16~~ | 🟢 **PARTIALLY ANSWERED 25 Aug — 11 of 40 rows now carry `Anyone else on the application?`** (was 0, unanswered since 18 Jul). Drives M4's dependent/individual checklist split. Still 29 blank, but every one of the 38 is pre-stamped `Checklist Filed` (D-352) so none blocks the import | M4 checklist variant | — | 🟢 |
